...
public class KajiPlayer extends AbstractPlayer {

	public KajiPlayer(String playerName) {
		super(playerName);
...
//全パターンでgetnextfieldだけで6でタイムアウト
	...
	public Action doMyTurn() {
		...
		int sum = 0 , sum2 = 0 , sum3 = 0 , sum4 =0 , SUM_CLEAR = 0 , iA=0 , m =1 , n =2; //ｎが連鎖数 , mが端数
		int sum_ojama=0;
		double delta_RL;
...
//getnextfieldで消えなかった場合のみ次のステップへ
					for(int i2=0 ; i2 < field.getWidth() ; i2++ ){
						for(int amount_of_DIRECTION2 = 0; amount_of_DIRECTION2<4 ; amount_of_DIRECTION2++){
							...
							next2field = nextfield.getNextField(puyo2, i2);
//消えちゃうなら考慮しない。めっちゃ消えるならそれでもいいかも。
							sum3 = SumField(next2field);
							if(sum3 == 0){
								dirA = dir;
								iA = i;
								break loop1;
							}
							if(sum2 - sum3 + 2  >= 4*n + m && sum2 - sum3 + 2 < SUM_CLEAR){//12個以上消えるなら(sum2は何もしないとsumより2大きい)
								dirA = dir;
								iA = i;
								SUM_CLEAR = sum2 - sum3 + 2;
			//					System.out.println("めっちゃ消すでー！");
								BIG_CLEAR = true;
								continue;
							}else if(sum - sum2 + 2 >= 4 ){
...
//２個先見ても消えなかった場合はこちらへ
		//ここからは点数計算に値する場合。
							int[][] connect2 = Get_connect(nextfield);
							for(int i_connect=0 ; i_connect < field.getWidth() ; i_connect++){
								for(int j=0 ; j < field.getTop(i_connect) ; j++){
									point_C += 1/2 * (connect2[i_connect][j]-1) * connect2[i_connect][j];
								}
							}
		//point2 = 隣接ぷよ数
							point2 = getNpuyoNumSum(nextfield, puyo2, i2);
		//point3 = MAX高さ
							point3 = 0;
							for(int j=0 ; j<field.getWidth() ;j++){
								if(next2field.getTop(j)+1 > point3)
								point3 = next2field.getTop(j)+1;
							}
		//point4 均し率。高いとダメ。凸凹ってこと。
							point4 = 0;
							for(int j=0 ;j<field.getWidth() ;j++){
								point4 +=Math.abs(next2field.getTop(j) - SumField(next2field) * 0.2);
							}
		//下に3個連結があるとポイントアップ
							point5 = Anspoint4(nextfield , connect2 , puyo2 , i2);
		//真ん中の方はあんまりおいてほしくない
							point6 = 0;
							if(SumField(nextfield) > 25 && ( i2 ==3 || i2 ==2)){
								point6 = -4;
							}
							point6_2 = 0;
		...
		//point7 = 3つ連結を横からつぶすのはマイナス
							point7 = 0;
							if((dir2 == PuyoDirection.UP || dir2 == PuyoDirection.DOWN) ){
								if(i2>0){
									if(connect2[i2-1][nextfield.getTop(i2)+1] == 3) point7 -= 10;
									if(connect2[i2-1][nextfield.getTop(i2)+2] == 3) point7 -= 10;
								}
								if(i2<5){
									if(connect2[i2+1][nextfield.getTop(i2)+1] == 3) point7 -= 10;
									if(connect2[i2+1][nextfield.getTop(i2)+2] == 3) point7 -= 10;
							...
							if(dir2 == PuyoDirection.RIGHT){
								if(i2>0){
									if(connect2[i2-1][nextfield.getTop(i2)+1] == 3) point7 -= 5;
								}
								if(i2<5){
									if(connect2[i2+1][nextfield.getTop(i2)+1] == 3) point7 -= 5;
								}
								if(connect2[i2][nextfield.getTop(i2+1)+1] == 3) point7 -= 5;
								if(i2<4){
									if(connect2[i2+2][nextfield.getTop(i2+1)+1] == 3) point7 -= 5;
								}
							}

							if(dir2 == PuyoDirection.LEFT){
								if(i2>0){
									if(connect2[i2-1][nextfield.getTop(i2)+1] == 3) point7 -= 5;
								}
								if(i2<5){
...
//point8は
							...
							if(dir2 != PuyoDirection.DOWN){
								if(nextfield.getPuyoType(i2, nextfield.getTop(i2)) == puyo2.getPuyoType(PuyoNumber.FIRST) &&
								nextfield.getPuyoType(i2, nextfield.getTop(i2)-1) == puyo2.getPuyoType(PuyoNumber.FIRST)	){
									point8 +=30;
								}
							}
							else{
								if(nextfield.getPuyoType(i2, nextfield.getTop(i2)) == puyo2.getPuyoType(PuyoNumber.SECOND) &&
								nextfield.getPuyoType(i2, nextfield.getTop(i2)-1) == puyo2.getPuyoType(PuyoNumber.SECOND)	){
									point8 +=30;
								}
							}
							if(dir2 == PuyoDirection.RIGHT){
								if(nextfield.getPuyoType(i2+1, nextfield.getTop(i2+1)) == puyo2.getPuyoType(PuyoNumber.SECOND) &&
								nextfield.getPuyoType(i2+1, nextfield.getTop(i2+1)-1) == puyo2.getPuyoType(PuyoNumber.SECOND)	){
									point8 +=30;
								}
							}
							if(dir2 == PuyoDirection.LEFT){
							...
							if(dir2 == PuyoDirection.DOWN || dir2 == PuyoDirection.UP){
								if(puyo2.getPuyoType(PuyoNumber.FIRST) == puyo2.getPuyoType(PuyoNumber.SECOND) ){
									point8 += 40;
								}
							}
//自分の下の方に同じ色のぷよが存在したら点数アップ
							point9 = 0;
							point9 += pre_point9(nextfield , i2 , puyo2.getPuyoType(PuyoNumber.FIRST));
							if(dir2 == PuyoDirection.DOWN || dir2 == PuyoDirection.UP){
								point9 += pre_point9(nextfield , i2 , puyo2.getPuyoType(PuyoNumber.SECOND));
							}else if(dir2 == PuyoDirection.RIGHT){
								point9 += pre_point9(nextfield , i2+1 , puyo2.getPuyoType(PuyoNumber.SECOND));
							}else
								point9 += pre_point9(nextfield , i2-1 , puyo2.getPuyoType(PuyoNumber.SECOND));

		//合計値
							sumNOW = point_C + Math.pow(10, point2) - 0.2 * Math.pow(point3, 2) - 1 *  point4 +
...
//point7 = 3つ連結を横からつぶすのはマイナス
					...
					if((dir == PuyoDirection.UP || dir == PuyoDirection.DOWN) ){
						if(i>0){
							if(connect[i-1][field.getTop(i)+1] == 3) point7 -= 10;
							if(connect[i-1][field.getTop(i)+2] == 3) point7 -= 10;
						}
						if(i<5){
							if(connect[i+1][field.getTop(i)+1] == 3) point7 -= 10;
							if(connect[i+1][field.getTop(i)+2] == 3) point7 -= 10;
...
//合計値
					sumNOW = Math.pow(10, point2) - 0.2 * Math.pow(point3, 2) - 1 *  point4 +
							0.3 * point5 + point6 + point6_2 + point7 + point8 + point9;
					if(sumNOW > sumP){
						sumP = sumNOW;
//						System.out.println(sumP);
...
//		Field field = getMyBoard().getField();
	...
	public static void main(String[] args) {
		AbstractPlayer player = new KajiPlayer("Kaji's agent");
