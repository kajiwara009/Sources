...
public class KajiGOD extends AbstractPlayer {

	public KajiGOD(String playerName) {
		super(playerName);
	}
//時々400でタイムアウト（ホントに時々）
	@Override
	public Action doMyTurn() {
		...
		int sum0 =0 , sum1 = 0 , sum2 = 0 , sum3 = 0 , sum4 =0 , SUM_CLEAR = 0 , iA=0 , m =0 , n =4; //ｎが連鎖数 , mが端数
		int CLEAR1 , CLEAR2 , CLEAR3 , minCLEAR = n * 3 + 4;
		int sum_ojama=0;
		double delta_RL;
		double sumP = -1000 , SumNow = 0;
		double point_C = 0 , point1 , point2 , point3 , point4 , point5 , point6,point6_2, point7, point8 ,point9;
		Action action = null;
...
/*大量消し*/			if(CLEAR1 > minCLEAR && CLEAR1 > SUM_CLEAR){
							dirA = dir;
							iA = i;
							SUM_CLEAR = CLEAR1;
							BIG_CLEAR = true;
...
/*大量消し*/					if(CLEAR2 > minCLEAR && CLEAR2 > SUM_CLEAR){
									dirA = dir;
									iA = i;
									SUM_CLEAR = CLEAR2;
									BIG_CLEAR = true;
...
/*大量消し*/							if(CLEAR3 > minCLEAR && CLEAR3 > SUM_CLEAR){
											dirA = dir;
											iA = i;
											SUM_CLEAR = CLEAR3;
											BIG_CLEAR = true;
...
//ここからは採点しますよー。
									...
									SumNow = POINT_CULC(nextfield2, i3, puyo3);
									if(SumNow > sumP){
										sumP = SumNow;
										dirA = dir;
										iA = i;
...
//fieldは置く前のものを指定する
	double POINT_CULC(Field field , int i , Puyo puyo ){
...
//point_isolateは連結無しのぷよの周りに多色ぷよがあったらマイナス(もとから負の値を返す)
	...
	public static void main(String[] args) {
		AbstractPlayer player = new KajiGOD("カジ神");
