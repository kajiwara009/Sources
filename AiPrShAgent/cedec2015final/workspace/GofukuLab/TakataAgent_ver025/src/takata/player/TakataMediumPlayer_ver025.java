////ファイル名：TakataSimpleMediumPlayer
////作成者：高田和磨
////作成日時：2014.12.10
////バージョン：0.1.1
////種類：霊能者
...
public class TakataMediumPlayer_ver025 extends AbstractMedium {
...
//	ArrayList<Agent> declaredMediumTellResultAgent = new ArrayList<>();
	...
	public String talk() {
		...
		if(getLatestDayGameInfo().getDay() == 0){
			...
			if(!isCameout && FakeMediumCOed){
				String CO = TemplateTalkFactory.comingout(getMe(), Role.MEDIUM);
				isCameout = true;
				return CO;
			}if(!isCameout && rand > 8){
				String CO = TemplateTalkFactory.comingout(getMe(), Role.MEDIUM);
				isCameout = true;
				return CO;
			}else if(OverList.size() + 3 < getLatestDayGameInfo().getAliveAgentList().size() && rand < 4){
				return Talk.SKIP;
			}else {
				return Talk.OVER;
			}
