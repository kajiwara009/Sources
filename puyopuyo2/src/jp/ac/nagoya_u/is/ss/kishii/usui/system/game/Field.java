...
public final class Field {
	...
	/**
	 * そこにぷよを置いたらプレイヤーが死亡してしまう段数を取得します。
	 * @return
	 */
	 ...
	 * @return
	 */
	public Field getNextField(Puyo puyo, int colmNumber) {
		PuyoDirection direction = puyo.getDirection();
	 ...
	 * @param colmNumber
	 */
	private void setHorizontalCurrentPuyo(Field nextField, Puyo puyo, int colmNumber) {
		PuyoType firstPuyoType =
				puyo.getPuyoType(PuyoNumber.FIRST);
		int firstTop = getTop(colmNumber);
		nextField.setPuyoType(colmNumber, firstTop + 1, firstPuyoType);

		int secondColmNumber = puyo.getSecondColmNumber();
		PuyoType secondPuyoType =
				puyo.getPuyoType(PuyoNumber.SECOND);
	 ...
	 * @param colmNumber
	 */
	private void setVerticalCurrentPuyo(Field nextField, Puyo puyo, int colmNumber) {
		int top = getTop(colmNumber);
