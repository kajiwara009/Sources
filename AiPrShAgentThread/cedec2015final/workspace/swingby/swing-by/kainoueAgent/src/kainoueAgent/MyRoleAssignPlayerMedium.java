...
public class MyRoleAssignPlayerMedium extends AbstractMedium {
//霊能師
	...
	List<Agent> med = new ArrayList<Agent>();
	...
	public String talk() {
		...
		if(!isComingOut){
			for(Judge judge: getMyJudgeList()){
				if(med.contains(judge.getTarget()) || judge.getResult() == Species.WEREWOLF || com){
	...
	public void updateTalkInfo(List<Talk> talkList){
		for(int i=0; i<talkList.size();i++){
			...
			case COMINGOUT:
				...
				if(!med.contains(talk.getAgent())){
					med.add(talk.getAgent());
				}
			...
			case DIVINED:
				...
				if(!med.contains(utterance.getTarget())){
					med.add(utterance.getTarget());
				}
				break;
			case INQUESTED:
				//霊能の結果
	...
	//	System.out.print("update--------------------------\n");
		...
		if(cnt.get(getMe())!=null && max == cnt.get(getMe()) && getDay()>1){
			com=true;
		}
	}
