...
public class MyRoleAssignPlayerBodyguard extends AbstractBodyguard {
	...
	Agent beforeGuard = null,GuaredAgent=null;
...
//		System.out.print(getLatestDayGameInfo().getGuardedAgent()+"--------------------------"+getMe()+"\n");
		if(buffer==null && getLatestDayGameInfo().getGuardedAgent()!=null && !whiteAgent.contains(getLatestDayGameInfo().getGuardedAgent())){
			whiteAgent.add(getLatestDayGameInfo().getGuardedAgent());
		}
	}
	...
	public void DEBUG(){
		...
		System.out.print("-------------DEATH AGENT-------------\n");
		for(int i=0;i<death.size();i++){
			System.out.print(death.get(i)+"\n");
		}
	}
	...
	public Agent selectGuardAgent(){
		if(GuaredAgent!=null) return GuaredAgent;
		List<Agent> co = comming_out,w=whiteAgent,al=alive;
		co.removeAll(blackAgent);	co.removeAll(death);	co.remove(getMe());
	...
	//	System.out.print("co--------------------------\n");
		...
		w.removeAll(death);	w.remove(getMe());	w.remove(am);
	//	System.out.print("w--------------------------\n");
		if(w.size()>0) return randomSelect(w);
		al.removeAll(blackAgent); al.remove(getMe());	al.remove(am);
	//	System.out.print("al--------------------------\n");
