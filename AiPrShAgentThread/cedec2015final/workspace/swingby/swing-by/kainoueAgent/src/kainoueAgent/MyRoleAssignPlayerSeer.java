...
public class MyRoleAssignPlayerSeer extends AbstractSeer {
	...
	boolean talk;
	List<Integer> talkCheck = new ArrayList<Integer>();
...
//		System.out.println("==============================talk=============================");
	//	System.out.println(isComingOut+"==============================talk============================="+com);
		if(!isComingOut){
			...
			//	System.out.println(com+" "+judge.getResult());
				if(judge.getResult() == Species.WEREWOLF || com){
	...
	public void updateJudge(){
		for(Judge judge: getMyJudgeList()){
			...
			case HUMAN:
				if(!whiteAgent.contains(judge.getTarget())) whiteAgent.add(judge.getTarget());
				break;
			case WEREWOLF:
				if(!blackAgent.contains(judge.getTarget())) blackAgent.add(judge.getTarget());
			}
		}
	}
	...
	public Agent divine() {
		// TODO 自動生成されたメソッド・スタブ
		//System.out.print("divine--------------------------\n");
		List<Agent> co = comming_out,al=alive;

		List<Agent> divineBlack=blackAgent;

		for(Judge judge: getMyJudgeList()){
			divineBlack.remove(judge.getTarget());
			al.remove(judge.getTarget());
			co.remove(judge.getTarget());
		}

		co.removeAll(whiteAgent);	co.removeAll(death);	co.remove(getMe());
		Map<Agent,Integer> cnt = get_cnt();
		int max=-1;
		Agent am=null;
		for(int i=0;i<alive.size();i++){
			Integer c = cnt.get(alive.get(i));
			if(c==null) c=0;
			if(c>max){
				c=max;
				am=alive.get(i);
		...
		divineBlack.remove(am); divineBlack.removeAll(death);
	//	System.out.print("divineBlack--------------------------\n");
		if(divineBlack.size()>0) return randomSelect(divineBlack);
	//	System.out.print("co--------------------------\n");
		co.removeAll(death); co.remove(am);
		if(co.size()>0) return randomSelect(co);

		al.removeAll(whiteAgent); al.remove(getMe());	al.remove(am);
	//	System.out.print("al--------------------------\n");
		if(al.size()!=0) return randomSelect(al);
		if(alive!=null && alive.size()!=0) randomSelect(alive);
		if(am!=null) return am;
		return getMe();
	}
