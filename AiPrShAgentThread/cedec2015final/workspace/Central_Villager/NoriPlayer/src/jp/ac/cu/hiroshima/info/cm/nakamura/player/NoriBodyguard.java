...
import jp.ac.cu.hiroshima.info.cm.nakamura.base.player.NoriAbstractBodyguard;
...
public class NoriBodyguard extends NoriAbstractBodyguard {
	...
	//占い師だと思っているエージェントのセット
	public void setmaybeseer(){
		List<Agent> AgentList = getLatestDayGameInfo().getAgentList();
		AgentList.remove(getMe());
		
		if(BlackList.size() != 0){
			if(maybeBlackList0.containsAll(BlackList)){
				maybeSEER = seerAgentList.get(0); 
			}
			else if(maybeBlackList1.containsAll(BlackList)){
				maybeSEER = seerAgentList.get(1); 
			}
			else if(maybeBlackList2.containsAll(BlackList)){
				maybeSEER = seerAgentList.get(2); 
			}
			else if(maybeBlackList3.containsAll(BlackList)){
				maybeSEER = seerAgentList.get(3); 
			}
			else if(maybeBlackList4.containsAll(BlackList)){
				maybeSEER = seerAgentList.get(4); 
			}
		}
	...
	public Agent guard() {
		...
		if(aliveAgentList.contains(maybeSEER)){
		guardAgent = maybeSEER;
		}else{
			double k = 0.0;
