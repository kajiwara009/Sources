...
import org.aiwolf.Satsuki.LearningPlayer.RandomPlayer;
 ...
 *
 */
public class ThreadMultiPlayerLearningStarter implements Runnable{
...
//	private static String className = IPARoleAssignPlayer.class.getName();
...
//					className = c.getName();
		...
		//複数スレッドの生成
		List<ThreadMultiPlayerLearningStarter> starter = new ArrayList<ThreadMultiPlayerLearningStarter>();
		for(int i = 0; i < threadNum; i++){
			starter.add(new ThreadMultiPlayerLearningStarter());
		}
...
//		System.out.println((System.currentTimeMillis() - timePre));
	...
	private static void output(int threadLoopCount, int threadNum) {
		for(Entry<Role, SituationPool> set: sitPoolMap.entrySet()){
			set.getValue().outputSituations("threadLearn/" + "multi" + "/" + ( (threadLoopCount+1) * loopPlayNum * threadNum), set.getKey());
		}
		for(Entry<Role, ObservePool> set: obsPoolMap.entrySet()){
			set.getValue().outputObserves("threadLearn/" + "multi" + "/" + ( (threadLoopCount+1) * loopPlayNum * threadNum), set.getKey());
		}
	}
...
//		playerMap.put(new PrShAssignPlayer(), Role.BODYGUARD);
		...
		names.add(WasabiRoleAssignPlayer.class.getName());
		names.add(ChipRoleAssignPlayer.class.getName());
		names.add(AIWolfMain.class.getName());
		Random rand = new Random();
		playerMap.put((Player) Class.forName(names.get(rand.nextInt(names.size()))).newInstance(), Role.POSSESSED);
		playerMap.put((Player) Class.forName(names.get(rand.nextInt(names.size()))).newInstance(), Role.WEREWOLF);
		playerMap.put((Player) Class.forName(names.get(rand.nextInt(names.size()))).newInstance(), Role.WEREWOLF);
		playerMap.put((Player) Class.forName(names.get(rand.nextInt(names.size()))).newInstance(), Role.WEREWOLF);
//		playerMap.put((Player) Class.forName(names.get(0)).newInstance(), Role.WEREWOLF);
//		playerMap.put((Player) Class.forName(names.get(1)).newInstance(), Role.WEREWOLF);
//		playerMap.put((Player) Class.forName(names.get(2)).newInstance(), Role.WEREWOLF);
