...
//	GiftedObserve forthMedium;
	...
	private Map<Situation, Integer> situationMap = new HashMap<Situation, Integer>();
...
/*	private SituationPool situationPool;
	...
	public Observe(StringTokenizer token, SituationPool situationPool){
		try {
			int count = 0;
			while (token.hasMoreTokens()) {
				if(count < 6){
					...
					situationMap.put(Situation.decodeHash(hash, situationPool), appearance);
				}
				count++;
			}
	...
	public String toDataString(){
		...
		for(Entry<Situation, Integer> set: situationMap.entrySet()){
			builder.append(set.getKey().hashCode() + "," + set.getValue() + ",");
		}
		return builder.toString();
	}

	public Map<Situation, Integer> getSituationMap() {
		return situationMap;
	}

	public void setSituationMap(Map<Situation, Integer> situations) {
...
*/	}
	...
	public void updateSituationMap(Situation situation){
		int value;
		if(situationMap.containsKey(situation)){
			value = situationMap.get(situation) + 1;
		}else{
			situationMap.put(situation, 0);
			value = 1;
		}
		situationMap.put(situation, value);
/*		
