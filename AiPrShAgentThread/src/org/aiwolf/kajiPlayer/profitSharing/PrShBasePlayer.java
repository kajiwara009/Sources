...
//	protected String filePath = "kajiwara009/kajiwara009_CEDEC2015/resources/";
	protected String inputDir = "default/100/";
	protected String filePath = "70000datas/";
	...
	protected String inputFileNameObserve = "outputO.txt";
	protected String inputFileNameSituation = "outputS.txt";
...
//	protected Map<Information, SelectAction> selectActionRecord = new HashMap<Information, SelectAction>();
	...
	protected LearningPool learningPool = new LearningPool();
	...
	private void inputChecker(){
		...
		int trueNum = 0;
		int falseNum = 0;
		for(Observe obs: observePool.getObserves().values()){
			for(Situation situation: obs.getSituationMap().keySet()){
				//Situation sit = Situation.decodeHash(hash);
				if(situationPool.getSituation(situation) != null){
					trueNum++;
				}else{
					falseNum++;
				}
		...
		System.out.println("true:" + trueNum + "  false:" + falseNum);
	}
	...
	public PrShBasePlayer(Role role) {
		myRole = role;
		if(isImport){
			situationPool.importSituations(inputDir, role);
			observePool.importObserves(inputDir, role, situationPool);
		}
		
//		inputChecker();
	}
	
	public void outputData(String dir){
		observePool.outputObserves(dir, getMyRole());
		situationPool.outputSituations(dir, getMyRole());
	}
...
//		Set<SelectAction> selActs = SelectAction.getAbleSelectActions(info);
	...
	public void finish() {
		if(isLearning){
			putLearningRes(coActionRecord);
			putLearningRes(selectActionRecord);
//			learnAction(coActionRecord);
//			learnAction(selectActionRecord);
		}
	}
	
	private <T> void putLearningRes(Map<Information, T> map) {
		for(Entry<Information, T> set: map.entrySet()){
			...
			SituationLearnResource slRes = new SituationLearnResource(getMyRole(), sit, set.getValue(), isWin());
			ObserveLearnResource olRes = new ObserveLearnResource(getMyRole(), obs, sit);
//			learningPool.getObsLearnRes().add(olRes);
			learningPool.addObsLearnRes(olRes);
//			learningPool.getSitLearnRes().add(slRes);
			learningPool.addSitLearnRes(slRes);
		}
